...
Knot* DatabaseInfo::AddColumn(string pDatabaseName, string pDatabaseColumn, string pColumnTypeOfData) {
    ...
    if (NewNode != NULL) {
        NewNode->TableName = pDatabaseName;          // Инициализация имени бд куда записываем новый столбик
        NewNode->ColumnName = pDatabaseColumn;       // Инициализация имени колонки
        NewNode->ColumnIndexationCheck = pColumnTypeOfData;   // Инициализация типа данных колонки
...
void DatabaseInfo::FindAllDatabaseColumns(const DatabaseInfo& pDatabaseInfo, string pDatabaseName)
{
    ...
    for (const Knot* p = pDatabaseInfo.begin(); p != NULL; p = p->next)
    {
        ...
        {
            DatabaseColumns = DatabaseColumns + "|" + p->ColumnName;
        }
    }
...
Knot* DatabaseInfo::end  (void) const { return tail; }
