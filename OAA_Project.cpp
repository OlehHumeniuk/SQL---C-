...
void PrintOurDatabaseHellper(DatabaseInfo *pDatabaseInfo, DatabaseInsert *pDatabaseInsert, string pDatabaseName)
{
    ...
    std::cout << endl;
    PrintOurDatabase(&DatabaseColumns, pDatabaseInsert, &lColumns);
}
void Vivod()
{

    DatabaseInfo DatabaseInfo_1;
    //DatabaseInfo_1.AddColumn("Table Name", "Column Name", "Type of data");
    DatabaseInfo_1.AddColumn("Person", "PersonID", "string");
    DatabaseInfo_1.AddColumn("Person", "FirstName", "string");
    DatabaseInfo_1.AddColumn("Person", "LastName", "string");
    DatabaseInfo_1.AddColumn("PersonType", "PersonTypeID", "Int");
    DatabaseInfo_1.AddColumn("PersonType", "PersonTypeName", "string");

    string DatabaseName = "Person";

    //PrintDatabaseColumn(std::cout, DatabaseInfo_1);//тут мы выводим все что было записано в клас информации по БД

    DatabaseInfo DatabaseColumns;
    map<int, string> lColumns;
    //FindAllDatabaseColumns_1(DatabaseInfo_1, DatabaseName, &lColumns, &DatabaseColumns);//выводим все возможные колонки с нашим именем бд
    std::cout << endl;
    //DatabaseInfo_1.FindAllDatabaseColumns(DatabaseInfo_1, DatabaseName);

    DatabaseInsert lDatabaseInsert;
    lDatabaseInsert.AddColumn(DatabaseName, "1", 1);
    lDatabaseInsert.AddColumn(DatabaseName, "Олег", 1);
    lDatabaseInsert.AddColumn(DatabaseName, "Гуменюк", 1);

    lDatabaseInsert.AddColumn(DatabaseName, "1", 2);
    lDatabaseInsert.AddColumn(DatabaseName, "Максим", 2);
    lDatabaseInsert.AddColumn(DatabaseName, "Борзуп", 2);

    lDatabaseInsert.AddColumn(DatabaseName, "1", 3);
    lDatabaseInsert.AddColumn(DatabaseName, "Линда", 3);
    lDatabaseInsert.AddColumn(DatabaseName, "Моршая", 3);

    lDatabaseInsert.AddColumn(DatabaseName, "1", 4);
    lDatabaseInsert.AddColumn(DatabaseName, "Джон", 4);
    lDatabaseInsert.AddColumn(DatabaseName, "Симпель", 4);
    //lDatabaseInsert.AddColumn(DatabaseName, "Maybe", 2);

    PrintOurDatabase(&DatabaseColumns, &lDatabaseInsert, &lColumns);
}
...
void AnalizArgumentsOfComandInsert(string pComand, string pNameTable)
{
    ...
    {
        TablesLinesInfo.insert(make_pair(pNameTable, NumberColumn));
    }
    int lCounter = 1;
    while (pComand[lNumberCurrentItemStringCommand + 1])// пока не конец строки
    {
...
void AnaliseComand(char buffer[255])//функцыя анализа нашей строки
{
    ...
    int lNumberCurrentItemString = 0, pNumberCurrentItem = 0, pIndex;
