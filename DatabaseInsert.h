...
{
    Node* next;
    string  TableName;       // Имя базыданных
...
public:// Спецификатор доступа public
    ...
    Node* AddColumn(string pDatabaseName, string pColumnName, string pColumnData, int pCounterOfLine);                                        //Добавление 
    Node* Insert(Node* pos, string pDatabaseName, string pColumnData, int pCounterOfLine);                                //Вставка перед pos
    void AddAllElementsDatabase(DatabaseInsert& lDatabaseInsert, int* pCounter, string pTableName, string pColumnName, string pColumnData);//Добавления всех колонок нашей базы данных
