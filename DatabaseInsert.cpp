...
//Добавление 
Node* DatabaseInsert::AddColumn(string pDatabaseName, string pColumnData, int pCounterOfLine)
{
    Node* NewNode = new (std::nothrow) Node();
    if (NewNode != NULL) {
        NewNode->DatabaseName = pDatabaseName;          // Инициализация имени бд куда записываем новый столбик
        NewNode->ColumnData = pColumnData;              // Инициализация типа данных колонки
...
void DatabaseInsert::AddAllElementsDatabase(DatabaseInsert& lDatabaseInsert, int* pCounter, string pDatabaseName, string pColumnData)
{
    lDatabaseInsert.AddColumn(pDatabaseName, pColumnData, *pCounter);
    (*pCounter)++;
}
